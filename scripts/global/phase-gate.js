...
function listChildren(epic, gh = ghJson) {
  return gh(['issue', 'list', '--state', 'all', '--limit', String(GH_ISSUE_LIMIT), '--json', 'number,title,body,state,labels'])
...
function checkMergeTarget(baseBranch, branchName, ticketNumber) {
  const failures = [];
  const rules = {
    'release': { target: 'main', desc: 'release branches must target main' },
    'hotfix': { target: 'main', desc: 'hotfix branches must target main' },
    'sandbox': { target: 'sandbox', desc: 'sandbox branches must target sandbox/*, not main' },
  };
  const [prefix] = branchName.split('/');
  const rule = rules[prefix];
  if (rule && baseBranch !== rule.target) {
    failures.push(`branch "${branchName}" violated rule: ${rule.desc} (got "${baseBranch}")`);
  }
  return failures;
}
...
module.exports = { parseBody, checkCreation, checkTransition, checkMergeTarget, logBypass, listChildren };
